package com.example.thyemleaffront.sec;
...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http
                ...
                .logout(logout->logout.logoutSuccessHandler(oidcLogoutSuccessHandler()).clearAuthentication(true).logoutSuccessUrl("/").permitAll().deleteCookies("JSESSIONID"))
                .oauth2Login(httpSecurityOAuth2LoginConfigurer -> httpSecurityOAuth2LoginConfigurer.loginPage("/oauth2Login").defaultSuccessUrl("/"))

                .exceptionHandling(httpSecurityExceptionHandlingConfigurer -> httpSecurityExceptionHandlingConfigurer.accessDeniedPage("/notAutorized"))
                .build();
